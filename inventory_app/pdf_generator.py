    ...
    def draw_table(
        ...
        for row in rows:
            ...
            for i, cell in enumerate(row):
                align = "L" if i == 0 or (len(headers) > 1 and i == 1 and headers[0] == "Дата") else "R"
                # для остатков: первая колонка наименование — влево
                if headers[0] in {"Товар", "Наименование"} and i == 0:
                    align = "L"
                if headers[0] == "Дата" and i <= 1:
                    align = "L"
                self.cell(col_widths[i], line_h, str(cell), border=1, align=align)
            self.ln(line_h)
...
def generate_movements_pdf(
    ...
    *,
    ...
    """PDF: движение товаров. Возвращает путь к файлу."""
    ...
    pdf = ReportPDF(
        "Движение товаров",
        f"Склад: {warehouse_name}. Период: {d_from} — {d_to}",
    )
    pdf.add_page()
    ...
    if not table_rows:
        table_rows = [["—", "Нет данных за период", "—", "—", "—", "—"]]

    pdf.draw_table(headers, table_rows, col_widths=[28, 90, 30, 30, 35, 30])
...
def generate_stock_pdf(
    ...
    *,
    ...
    if not table_rows:
        table_rows = [["Нет данных", "—"]]

    pdf.draw_table(headers, table_rows, col_widths=[200, 40])
    pdf.output(str(out_path))
    return out_path
